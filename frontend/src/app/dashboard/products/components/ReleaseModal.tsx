      ...
      <div className="w-full max-w-7xl rounded-[2.5rem] bg-white p-0 shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] border border-gray-100 overflow-hidden animate-in fade-in zoom-in-95 duration-300">
        ...
        <form onSubmit={onSubmit} className="p-6 space-y-6 bg-gray-50/30">
          <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-6 max-w-[1400px] mx-auto">
            ...
            <div className="space-y-6">
              <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-6">
                ...
                <div className="space-y-4">
                  <div className="group">
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1 group-focus-within:text-primary transition-colors">Release From Area</label>
                  ...
                  <div className="group">
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Issued To / Requested By</label>
                    <div className="relative">
                  ...
                  <div className="group">
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Destination / Purpose</label>
                    <div className="relative">
                      <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                      <input 
                        required 
              ...
              <div className="bg-gray-900 p-6 rounded-2xl border border-gray-800 text-white space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest">Logged Date</span>
                  </div>
                  <span className="text-xs font-black">{clientDate}</span>
                </div>
                <div className="pt-4 border-t border-gray-800">
                  <div className="flex items-center space-x-6 text-gray-300">
                    <div className="flex items-center space-x-2">
                      <span className="w-2 h-2 rounded-full bg-primary animate-pulse"></span>
                      <span className="text-[11px] font-black uppercase tracking-widest">Active Session</span>
                    </div>
            ...
            <div className="space-y-6 flex flex-col h-[600px]">
              ...
              <div className="relative group">
                ...
                <input 
                  type="text" 
                  placeholder="Scan SKU or type item name to add to release list..."
                ...
                {releaseSearchResults.length > 0 && (
                  <div className="absolute left-0 right-0 top-full mt-3 bg-white rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-gray-100 z-10 overflow-hidden animate-in slide-in-from-top-2 duration-200">
                    <div className="p-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                      <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Found {releaseSearchResults.length} Matching Items</span>
                      <button onClick={() => setReleaseSearchInput('')} className="text-[10px] font-black text-primary hover:underline">Clear Search</button>
                    </div>
                          ...
                          >
                            <div className="flex items-center space-x-4">
                              ...
                              <div>
                                ...
                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{product.description || 'No description available'}</p>
                              </div>
                            </div>
                            <div className="flex items-center space-x-6">
                              <div className="text-right">
                                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-0.5">Available In-Store</span>
                                <span className={`text-sm font-black ${available <= 0 ? 'text-red-500' : 'text-gray-900'}`}>{available} {product.unit}</span>
                              </div>
              ...
              <div className="flex-1 bg-white rounded-[2rem] border-2 border-gray-100 shadow-inner overflow-hidden flex flex-col">
                <div className="px-8 py-5 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
                  <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest">Release Manifest ({form.items.length} Items)</h3>
                  <button type="button" onClick={() => setForm({...form, items: []})} className="text-[10px] font-black text-red-400 hover:text-red-600 uppercase tracking-widest">Reset List</button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  {form.items.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-gray-300 space-y-4 opacity-50">
                      ...
                      <p className="text-sm font-bold uppercase tracking-widest">Manifest is currently empty</p>
                    </div>
                  ) : (
                    form.items.map(item => (
                      <div key={item.productId} className="flex items-center justify-between p-4 bg-gray-50 border border-gray-100 rounded-2xl group hover:border-primary transition-all animate-in slide-in-from-right-4">
                        <div className="flex items-center space-x-4 flex-1">
                          ...
                          <div>
                            <h4 className="text-sm font-black text-gray-900 leading-none mb-1">{item.name}</h4>
                            <div className="flex items-center space-x-3">
                              <span className="text-[10px] font-black text-gray-400 uppercase tracking-tighter">{item.available} Available</span>
                              <span className="text-gray-300">•</span>
                              <span className="text-[10px] font-black text-primary uppercase tracking-tighter">{item.unit}</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center space-x-8">
                          <div className="flex items-center bg-white border border-gray-100 rounded-xl p-1 shadow-sm">
                            <button 
                              ...
                              className="w-10 h-10 flex items-center justify-center text-gray-400 hover:text-primary hover:bg-gray-50 rounded-lg transition-all"
                            >-</button>
                            <input 
                              type="number" 
                              value={item.quantity}
          ...
          <div className="max-w-[1400px] mx-auto pt-6 flex justify-between items-center border-t border-gray-100">
            <div className="flex items-center space-x-3 text-xs text-gray-400 font-bold italic">
            ...
            <div className="flex items-center space-x-4">
              <button 
                type="button" 
                onClick={onClose} 
                className="px-10 py-5 text-xs font-black text-gray-400 uppercase tracking-widest hover:text-gray-900 transition-all"
              >
                Discard Manifest
              </button>
              <button 
                ...
                type="submit" 
                className="px-16 py-5 bg-[#50C878] text-white rounded-[1.5rem] text-xs font-black uppercase tracking-[0.2em] shadow-[0_20px_40px_rgba(80,200,120,0.3)] hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 disabled:grayscale"
              >
                {isProcessing ? (
                  <div className="flex items-center space-x-3">
                    <Loader2 className="h-4 w-4 animate-spin" />
                    <span>Processing Release...</span>
                  </div>
                ) : 'Complete Quick Pull'}
              </button>
            </div>
