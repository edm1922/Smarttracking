...
import { Search, Filter, Printer, Trash2, Box, Users, LayoutTemplate, LogIn } from 'lucide-react';
...
export const PayrollCredentialsTab: React.FC<PayrollCredentialsTabProps> = ({
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      {/* Search & Bulk Section */}
      <div className="flex flex-col md:flex-row items-end gap-6 bg-white p-10 rounded-3xl border border-gray-200 shadow-xl shadow-gray-200/50 no-print">
        <div className="flex-1 space-y-3">
          <label htmlFor="personnel-search" className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1 flex items-center gap-2">
        ...
        <div className="w-full md:w-auto space-y-3">
          <label htmlFor="affiliation-filter" className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Affiliation</label>
          <select 
      ...
      {/* Portal Login Notice */}
      <div className="bg-primary/5 border border-primary/20 rounded-[2rem] p-6 flex items-center justify-between gap-6 no-print">
        <div className="flex items-center gap-4">
          <div className="h-12 w-12 bg-primary text-white rounded-2xl flex items-center justify-center shrink-0">
            <LogIn className="h-6 w-6" />
          </div>
          <div>
            <p className="text-sm font-bold text-gray-900">Portal Login</p>
            <p className="text-xs text-gray-500">In order to login, please access the portal at:</p>
          </div>
        </div>
        <div className="flex items-center gap-3 shrink-0">
          <code className="text-xs font-mono font-bold text-primary bg-white px-4 py-2.5 rounded-xl border border-primary/20 whitespace-nowrap">https://www.centroservicescooperative.online/portal</code>
          <button
            onClick={() => {
              navigator.clipboard.writeText('https://www.centroservicescooperative.online/portal');
              setCopied(true);
              setTimeout(() => setCopied(false), 2000);
            }}
            className="h-11 px-5 bg-primary text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:brightness-110 transition-all flex items-center gap-2"
          >
            {copied ? 'Copied!' : 'Copy'}
          </button>
        </div>
      ...
      <div className="print-cards-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {loadingUsers ? (
          ...
          filteredUsers.map(user => (
            <div key={user.id} className="bg-white border border-gray-100 rounded-3xl p-8 shadow-xl shadow-gray-200/50 flex flex-col items-center text-center relative overflow-hidden group hover:border-primary/20 hover:-translate-y-2 transition-all duration-300">
              {!isStaff && (
                <button 
                  onClick={() => onDeleteEmployee(user.id)}
                  aria-label={`Revoke access for ${user.fullName}`}
                  className="absolute top-6 left-6 p-4 bg-red-50 text-red-500 rounded-2xl opacity-0 group-hover:opacity-100 transition-all hover:bg-red-500 hover:text-white z-10 no-print"
                >
              ...
              <div className="w-full bg-gray-50/50 rounded-3xl p-6 text-left border border-gray-100/50 space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-[9px] font-black uppercase text-gray-500 tracking-[0.2em]">Portal ID</span>
                  <code className="text-xs font-black text-gray-900 tracking-tighter">{user.username.toLowerCase()}</code>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[9px] font-black uppercase text-gray-500 tracking-[0.2em]">Passkey</span>
                  <code className="text-xs font-mono font-bold text-gray-500 bg-white px-2 py-1 rounded-lg border border-gray-100">{user.sys_id?.replace(/-/g, '').toLowerCase()}</code>
                </div>
              </div>

              <p className="text-[8px] font-black text-gray-500 mt-6 tracking-[0.2em] uppercase italic no-print">Smart Portal Access</p>
            </div>
          ))
        ) : (
          <div className="col-span-full py-40 text-center space-y-6">
            <div className="h-24 w-24 bg-gray-50 rounded-2xl flex items-center justify-center mx-auto border-2 border-dashed border-gray-100">
              <Users className="h-10 w-10 text-gray-200" />
            </div>
            <div className="text-center">
              <p className="text-sm font-black text-gray-500 uppercase tracking-widest">No matching personnel found</p>
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-tighter mt-1 italic">Try adjusting your search criteria or filter</p>
            </div>
          </div>
        )}
      </div>
